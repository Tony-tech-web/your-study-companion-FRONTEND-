        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4 space-y-3">
          <div className="flex gap-2">
            ...
            <button onClick={handleSearch} disabled={searching || !query.trim()}
              className="px-5 py-2.5 rounded-lg text-sm font-semibold text-white disabled:opacity-40 transition-all hover:opacity-90 active:scale-95"
                    ...
                    {(['APA', 'MLA', 'CHI'] as const).map(f => (
                      <button key={f} onClick={() => setCitationFormat(f)}
                        className={cn('flex-1 py-1.5 text-[11px] font-semibold rounded-md transition-all',
                          citationFormat === f ? 'bg-[var(--primary)] text-white shadow-sm' : 'text-[var(--muted)] hover:text-[var(--foreground)]')}>
                        {f}
                      </button>
                    ))}
                  </div>

                  <div className="p-3 bg-[var(--input)] rounded-lg border border-[var(--border)]">
